    ...
    public static class misc
    {
        ...
        public static string ByteAToStringHex(byte[] val, string split)
        {
            ...
            for (int index = 0; index < val.Length; ++index)
                str = str + val[index].ToString("X2") + split;
            if (split.Length > 0)
        ...
        public static void SaveFile(string file, string asm)
        {
            ...
            if (!Directory.Exists(file))
                Directory.CreateDirectory(DirOf(file));
        ...
        public static uint[] GrabMemory(string filename, uint start, int length, uint offset)
        {
            ...
            uint[] numArray1 = length / 4 > 0 ? new uint[length / 4] : new uint[1];
            int index = 0;
            int sourceIndex = 0;
            while (sourceIndex < buffer.Length)
            {
                byte[] numArray2 = new byte[length1];
                Array.Copy((Array)buffer, sourceIndex, (Array)numArray2, 0, length1);
                Array.Reverse((Array)numArray2);
                if (length1 == 3 || length1 == 4)
                    numArray1[index] = BitConverter.ToUInt32(numArray2, 0);
                else if (length1 == 2)
                    numArray1[index] = (uint)BitConverter.ToUInt16(numArray2, 0);
                else if (length1 == 1)
                    numArray1[index] = (uint)numArray2[0];
                ++index;
                sourceIndex += 4;
            }
            return numArray1;
        ...
        public static string[] LoadCWP3(string file)
        {
            ...
            if (File.Exists(file))
            {
                ...
                int num = 1;
                foreach (string contents in str.Split(separator, (StringSplitOptions)num))
                {
                    if (contents != "")
                    {
